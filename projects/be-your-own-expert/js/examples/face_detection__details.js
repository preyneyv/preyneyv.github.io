...
export const configureExample = (brfv5Config) => {
  ...
  // ... or further away.
  const __config_onlyDetectFarFaces   = false && !__config_onlyDetectNearFaces;

  // Filter all rectangles that don't belong to a merged rectangle.
  const __config_filterNoise          = false;
  ...
  if(__config_onlyDetectFarFaces) { // detect small faces
    ...
    fdConfig.minFaceSize      =  12 * scaleFactor; // very small min face size
